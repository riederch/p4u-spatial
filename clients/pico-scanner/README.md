# P4U Spatial Scanner for picoVrVr

This is the first real Android application shell for the picoVrVr headset.
...
- Android/picoVrVr interactive package installation.
...
The debug APK can be installed using the normal administrator-controlled picoVrVr/Android sideload path for development.
...
The canonical production procedure is documented in [the picoVrVr Scanner release runbook](../../docs/pico-scanner-release.md). GitHub Actions workflow `Publish picoVrVr Scanner APK` builds, signs, verifies and publishes the APK plus `release-descriptor.json`. Publishing that descriptor to a Bridge remains an explicit administrator action.
...
The headset likewise has a **local device name** configured on the picoVrVr. It is sent during pairing as the device-provided name. A Bridge administrator may independently assign a **global device name** to the paired device. The Bridge global name takes precedence in Bridge-side presentation without changing the picoVrVr-local name.
...
The picoVrVr UI allows both values to be changed after pairing for site-specific networking. A changed address must resolve through discovery to the same stored Bridge `instanceId` before it is trusted for authenticated traffic. Local overrides do not alter server-side Bridge configuration.
