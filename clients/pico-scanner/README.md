...
The PICO UI allows both values to be changed after pairing for site-specific networking. A changed address must resolve through discovery to the same stored Bridge `instanceId` before it is trusted for authenticated traffic. Local overrides do not alter server-side Bridge configuration.
