 ...
 * picoVr runtime can expose whichever camera it permits to normal Android applications.
...
class PicoQrScanner(
    ...
    fun start() {
        ...
        if (cameraId == null) {
            fail("Keine Android-Kamera wurde vom picoVr-System für diese App freigegeben.")
            return
        }
        ...
        try {
            manager.openCamera(cameraId, object : CameraDevice.StateCallback() {
                    ...
                    device.createCaptureSession(
                        ...
                        object : CameraCaptureSession.StateCallback() {
                            ...
                            override fun onConfigureFailed(captureSession: CameraCaptureSession) {
                                fail("picoVr-Kamera konnte nicht konfiguriert werden.")
                            }
                        },
                        handler,
                    )
                ...
                override fun onDisconnected(device: CameraDevice) {
                    device.close()
                    fail("picoVr-Kamera wurde getrennt.")
                }
                override fun onError(device: CameraDevice, error: Int) {
                    device.close()
                    fail("picoVr-Kamera meldet Fehler $error.")
                }
            }, handler)
